...
############################################################################
#' Simulate a spatial capture-recapture data set where individuals move according to a biased random walk
#'
...
#' @param rho A number >= 0 and <= 1; The parameter describing the strength of attraction to the source of bias/activity center relative to the past location
...
#'
#' @export
simData_BRW <- function(K.train, K.forecast, det.sd, p0, move.sd, rho = 0.5, 
  ...
  if(moveDist == "Laplace"){
    for(i in 1:N){
      ...
      UPix[i,1] = which(rmultinom(1, 1, cellProbs.std) == 1)
      ...
      for(k in 2:K){
        dist.vec = sqrt( ( rho * habitat[SPix[i],1] + (1-rho) * habitat[UPix[i,k-1],1] - habitat[,1] )^2 +
                           ( rho * habitat[SPix[i],2] + (1-rho) * habitat[UPix[i,k-1],2] - habitat[,2] )^2 )
