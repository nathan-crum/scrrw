...
#############################################
#############################################
#' Fit a closed population spatial capture-recapture model with a biased random walk and line transect sampling
#'  
...
#' @param MOVEMENT.MODEL Character string, must be "RWAC" for random walk with activity center, i.e., biased random walk
#'
#' @export
scr_lt_Closed_BiasedRW = function(data, density.movement = ~1, g0 = ~1, det.scale = ~1, hSigma = ~-1, hBeta = ~-1, MOVEMENT.MODEL = "RWAC"){
  ...
  par.out = SCR_lt_Closed_BiasedRW_inits(data)
  ...
  data$rho.init.indices = par.out$rho.init.indices
  
  if(MOVEMENT.MODEL == "RWAC"){
    results.BFGS <- optim(par = par.out$par, fn = SCR_lt_Closed_BiasedRW_loglklhd, data = data, hessian = T,
...
#########################################################
#########################################################
#' Generate initial values for the spatial capture-recapture biased random walk model
#' 
#' @param data Named list of input data and parameter indices; passed from scr_lt_Closed_BiasedRW
#'
#' @export
SCR_lt_Closed_BiasedRW_inits = function(data){
  ...
  # rho initial values
  rho.init.indices = length(par) + 1
  par[rho.init.indices] = 0 #qlogis(0.5)
  ...
  return(list(par = par, d.init.indices = d.init.indices, m.init.indices = m.init.indices, 
              ...
              rho.init.indices = rho.init.indices))
...
###################################################
###################################################
#' Likelihood function for the closed population, biased random walk, spatial capture-recapture model
#' 
#' @param data Named list of input data; passed from scr_lt_Closed_BiasedRW
#' @param par Numeric vector of parameters; passed from scr_lt_Closed_BiasedRW
#'
#' @export
SCR_lt_Closed_BiasedRW_loglklhd = function(data, par){  
...
#  p.dot = 1 - sum(p.no.det.tot[,,K])
  ...
  ind.likelihood = C_indLklhd_RWAC(N = N, K = K, nCells = nCells, nACCells = nACCells,
                                         tr_b4_occ = tr_b4_occ, 
